...
{-
    ...
    și transformare este reprezentată sub forma ugit@github.com:tudor-paraschivescu/HaliteBot.gitnui arbore sintactic
...
toTransformation transformation = case transformation of
    Translation x y -> S.translation x y
    Scaling factor -> S.scaling factor
    Combine transformations -> S.combineTransformations [toTransformation trans | trans <- transformations]
...
toRegion region = case region of
    ...
    Rectangle width height -> S.rectangle width height
...
-- If the first 2 transformations are the same, combine them
fuseTransformations (transformation1 : transformation2 : rest) =
    case (transformation1, transformation2) of
        (Translation x1 y1, Translation x2 y2) ->
            fuseTransformations $ Translation (x1 + x2) (y1 + y2) : rest
        (Scaling factor1, Scaling factor2) ->
            fuseTransformations $ Scaling (factor1 * factor2) : rest
        -- The transformation is of type Combine    
        _ ->
            transformation1 : fuseTransformations (transformation2 : rest)
