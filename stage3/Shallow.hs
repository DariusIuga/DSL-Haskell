...
-}
bfs :: (Ord a, Num b) => a -> (a -> [a]) -> [(a, b)]
bfs start expand =
    let
        result = helper [(start, 0)] empty
        helper [] _ = []
        helper ((state, distance) : states) visited = if member state visited then helper states visited else (state, distance) : helper stateQueue alreadyVisited
            where
                stateQueue = states ++ [(n, distance + 1) | n <- expand state, not (member n visited)]
                alreadyVisited = insert state visited
    in result
